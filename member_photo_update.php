...
if(!$member_id=is_login($link)){
	skip('login.php', 'onFocus.gif', '请登录之后再对自己的头像做设置!');
}
...
if(isset($_POST['submit'])){
	...
	if($upload['return']){
		...
		if(mysqli_affected_rows($link)==1){
			skip("member.php?id={$member_id}",'onCorrect.gif','头像设置成功！');
		}else{
			skip('member_photo_update.php','onError.gif','头像设置失败，请重试');
		}
	}else{
		skip('member_photo_update.php', 'onError.gif',$upload['error']);
	}
}
//SUB_URL.
?>
