...
$page：分页的get参数
*/
function page($count,$page_size,$num_btn=8,$page='page'){
	if(!isset($_GET[$page]) || !is_numeric($_GET[$page]) || $_GET[$page]<1){/*当前页码修正*/
		$_GET[$page]=1;
	}
	if($count==0){//如果没有记录，返回空的字符串
		$data=array(
			'limit'=>'',
			'html'=>''
		);
	...
	if($_GET[$page]>$page_num_all){/*当前页码修正*/
	...
	if(isset($arr_current['query'])){
		parse_str($arr_current['query'],$arr_query);//将字符串解析成多个变量
	...
	$html=array();
	if($num_btn>=$page_num_all){/*当前页码修正*/
		//把所有的页码按钮全部显示
		...
		$num_left=floor(($num_btn-1)/2);//num_left左边按钮数
		...
		for($i=0;$i<$num_btn;$i++){
			if($_GET[$page]==$start){
				$html[$start]="<span>{$start}</span>";//$html[$start]开始页码号
			}else{
				$html[$start]="<a href='{$url}{$start}'>{$start}</a>";
			}
			$start++;
		}
		//如果按钮数目大于等于4的时候做省略号效果
		if(count($html)>=4){
			reset($html);
			$key_first=key($html);
			end($html);
			...
			if($key_first!=1){
				...
				array_unshift($html,"<a href='{$url}=1'>1...</a>");//插入单元
			}
			if($key_end!=$page_num_all){
				array_pop($html);//将数组末尾的单元移出数组
				array_push($html,"<a href='{$url}{$page_num_all}'>...{$page_num_all}</a>");//插入单元
			}
		}
	}
	if($_GET[$page]!=1){
